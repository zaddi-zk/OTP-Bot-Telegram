...
# (Legacy — audio generation removed; Vapi handles TTS)
...
def send_support_menu(chat_id: int, message_id: Optional[int] = None) -> None:
    ...
    buttons = types.InlineKeyboardMarkup(row_width=1)
    buttons.add(types.InlineKeyboardButton("💬 Contact Admin", url="https://t.me/N_onie"))
    buttons.add(types.InlineKeyboardButton("↩ Back", callback_data="back_to_menu"))
    if message_id:
        try:
            bot.edit_message_text(text, chat_id, message_id, reply_markup=buttons, parse_mode="HTML")
...
def send_channel_menu(chat_id: int, message_id: Optional[int] = None) -> None:
    ...
    buttons = types.InlineKeyboardMarkup(row_width=1)
    buttons.add(types.InlineKeyboardButton(f"📢 Main Channel", url=MAIN_CHANNEL_URL))
    buttons.add(types.InlineKeyboardButton(f"📣 Backup Channel", url=BACKUP_CHANNEL_URL))
    buttons.add(types.InlineKeyboardButton("↩ Back", callback_data="back_to_menu"))
    if message_id:
        try:
            bot.edit_message_text(text, chat_id, message_id, reply_markup=buttons, parse_mode="HTML")
    ...
    if call.data == "normal_confirm":
        ...
        if call_mode_label:
            try:
                ...
                logger.warning("Failed to remove call_mode_label.txt for user %s: %s", user_id_str, exc)
    ...
    if call.data.startswith("voice_select_"):
        ...
        if name:
            ...
            if state == "crack_blast_step_4_voice":
                ...
                write_user_file(user_id_str, "crack_voice_provider.txt", provider)
            ...
            if state == "custom_call_step_5_voice":
                ...
                write_user_file(user_id_str, "custom_voice_provider.txt", provider)
